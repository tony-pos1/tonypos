// Synthesizer sound cues using Web Audio API (100% offline, zero network assets)
class SoundManager {
  ...
  // Subtle tap sound when adding item or pressing numbers
  playTap() {
    try {
  ...
  // Success payment cash register chime
  playCashRegister() {
    try {
      const ctx = this.getContext();
      if (!ctx) return;
